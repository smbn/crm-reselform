   ...
   <div class="mbot10">
      <?php if($invoice->is_recurring_from == null
         ...
         <?php } else if($show_recurring_invoice_info){ ?>
            <span class="label label-default inline-block padding-5 mbot5">
               <?php
            ...
            if($recurring_invoice->cycles == 0 || $recurring_invoice->cycles != $recurring_invoice->total_cycles){
               echo '<span class="label label-default inline-block padding-5 mleft5 "><i class="fa fa-question-circle fa-fw" data-toggle="tooltip" data-title="'._l('recurring_recreate_hour_notice',_l('invoice')).'"></i> ' . _l('next_invoice_date','<b>'._d($next_date).'</b>') .'</span>';
            }
         }
         ?>
      </div>
