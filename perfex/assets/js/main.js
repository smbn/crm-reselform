   ...
   $('body').on('hide.bs.modal', '#task-modal', function(e) {
    ...
    if (table_invoices.length > 0 || table_estimates.length > 0) {
        ...
        $.each(Sales_table_Filter, function () {
            Sales_table_ServerParams[$(this).attr('name')] = '[name="' + $(this).attr('name') + '"]';
        });

        if (table_invoices.length) {
            // Invoices tables
            initDataTable(table_invoices, (admin_url + 'invoices/table' + ($('body').hasClass('recurring') ? '?recurring=1' : '')), 'undefined', 'undefined', Sales_table_ServerParams, !$('body').hasClass('recurring') ? [
                [3, 'desc'],
                [0, 'desc']
        ...
        if (table_estimates.length) {
            // Estimates table
            initDataTable(table_estimates, admin_url + 'estimates/table', 'undefined', 'undefined', Sales_table_ServerParams, [
                [3, 'desc'],
                [0, 'desc']
            ]);
        }
